...
# in-source fallback `var version = "0.5.0-dev"` in cmd/uboot/main.go
# so that local `docker build` without --build-arg produces a coherent
# `0.5.0-dev` binary. Tagged releases pass UBOOT_VERSION=<version> via
# `make build VERSION=<version>` from the publish.yml workflow.
ARG UBOOT_VERSION=0.5.0-dev
