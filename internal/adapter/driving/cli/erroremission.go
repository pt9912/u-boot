...
// subcommand RunE (slice-v1-cli-json-dry-run-init T0-(e)).
//
...
// gesetzt (`mapErr := mapAddErrorToDiagnostic` für add, ana log für
// init).
//
// Verhalten:
...
//
//nolint:unparam // command/mapErr have only one caller (add) until
// init's T5 migration; both are load-bearing parameters of the
// generic helper.
func reportError(
	out io.Writer,
	err error,
