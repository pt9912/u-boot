...
func mapInitErrorToDiagnostic(err error) diagnosticItem {
	...
	// slice-later-local-templates T1 + Pre-T5-Review: harmonise the four
	// template-init sentinels so closely-related failures carry a
	// consistent code AND the code-class matches the exit-class
	// (cf. Review-Round-9 #1 — Code-Class == Exit-Class). Dual-classifier
	// partners of isTemplateInitValidationError / isFilesystemError in
	// cli.go; kept in sync so the envelope code and ExitCode never
	// diverge. Before this, not-found/invalid-path fell to the
	// LH-FA-CLI-006 default and render-failures showed CLI-006 at exit 14.
	case errors.Is(err, driving.ErrTemplateNotFound):
		// Bad `--template` selection (catalog name or local path not
		// found) → LH-FA-TPL-001 (the `init --template <ref>` req), exit 10.
		return diagnosticItem{Level: "error", Code: "LH-FA-TPL-001", Message: err.Error()}
	case errors.Is(err, driving.ErrInvalidTemplatePath), errors.Is(err, driving.ErrTemplateInvalid):
		// Template content invalid: malformed template.yaml (metadata) or
		// an unsafe file path inside the tree (`..`/absolute/symlink) →
		// LH-FA-TPL-002, exit 10.
		return diagnosticItem{Level: "error", Code: "LH-FA-TPL-002", Message: err.Error()}
	case errors.Is(err, driving.ErrTemplateRender):
		// Technical render/IO failure → exit 14; code must match that
		// class. Shares LH-NFA-REL-003 with ErrInitFileSystem — both are
		// the technical-reliability surface.
		return diagnosticItem{Level: "error", Code: "LH-NFA-REL-003", Message: err.Error()}
