...
- `InitProjectUseCase` — `LH-FA-INIT-001..007`. Sentinels:
...
- `DoctorUseCase` — `LH-FA-DIAG-001..004`. Keine Sentinels
  (Befunde sind im `domain.DiagnosticReport`).
- `AddServiceUseCase` — `LH-FA-ADD-001..002`, `LH-FA-ADD-005`.
  Sentinels: `ErrServiceUnsupported`, `ErrServiceInconsistent`.
- `UpUseCase` — `LH-FA-UP-001..003`. Sentinels:
...
- `DownUseCase` — `LH-FA-UP-004`. Sentinel:
  `ErrConfirmationRequired` (CLI-Code 10 für §254-destruktive-Aborts).

## Geplante Erweiterungen

- `GenerateUseCase` — `LH-FA-GEN-001..005`.
- `ConfigUseCase` — `LH-FA-CONF-001..005`.
