...
- Geschäftslogik (z. B. *„darf ein Service idempotent reaktiviert
  werden?"*, `LH-FA-ADD-005`) verteilt sich auf Commands und Helper.
...
- `LH-FA-ARCH-001` – Hexagonales Pattern (Pflicht).
- `LH-FA-ARCH-002` – Schichten und Verzeichnislayout.
- `LH-FA-ARCH-003` – Import-Regeln und Enforcement via `depguard`.
- `LH-NFA-MAINT-001` – modulare Architektur.
- `LH-NFA-MAINT-003` – fachliche Funktionen automatisiert testbar.
...
- Wiring-Schicht ausschließlich in `cmd/uboot/` (`LH-FA-BUILD-009`).
- Import-Regeln aus `LH-FA-ARCH-003` werden im `lint`-Stage per
...
fachlichen Slice scharf geschaltet (M3 `u-boot init`).
...
- **Einarbeitung:** Beitragende ohne Hex-Erfahrung brauchen einmal das
  ...
  aus `LH-FA-ARCH-003`. `depguard`-Fehlermeldungen sind dabei
  selbsterklärend.
...
- **Clean Architecture / Onion Architecture:** semantisch nah am Hex,
  ...
  org-weit.
- **Schicht-loser Layered-CLI-Stil:** würde gegen `LH-NFA-MAINT-001`
  (modulare Architektur) und `LH-NFA-MAINT-003` (Testbarkeit)
  arbeiten.
