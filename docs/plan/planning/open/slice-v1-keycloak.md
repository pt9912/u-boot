...
| T | Inhalt |
| - | ------ |
| T1 | **Catalogue + Templates + T1-Sub-Decisions.** Catalogue erweitern (`isSupportedService("keycloak")` + `supportedServices()` returns `["postgres", "keycloak"]`); drei Templates anlegen — `templates/services/keycloak.compose.tmpl` (Image `quay.io/keycloak/keycloak:26.0` — T1-Decision LTS-Pin; Port-Mapping `8080:8080`; Healthcheck-Entscheidung dokumentiert: bevorzugt `curl --fail http://localhost:9000/health/ready` mit `--health-cmd` analog Postgres-Template, sonst weglassen; Env-Block; `start: dev` als Compose-Command für die LH-AK-003-Boot-Variante), `keycloak.env.tmpl` (`KEYCLOAK_ADMIN=CHANGEME_KEYCLOAK_ADMIN` + `KEYCLOAK_ADMIN_PASSWORD=CHANGEME_KEYCLOAK_ADMIN_PASSWORD`), **kein** `keycloak.volume.tmpl` (siehe T2 Skip-Volume). **T1-Sub-Decision: Template-Parametrisierung bleibt inline-hardcoded** wie heute Postgres (`templateData{}` leer), kein `{{ .Name }}`-Refactor — falls späterer Image-Tag-Override (z. B. via `services.keycloak.imageTag`) gewünscht, ist das eigener Slice. `embed.FS`-Eintrag in `templates.go` für Template-Resolution erweitern, dabei den `services/postgres.volume.tmpl`-Required-List-Check anpassen (Volume-Template wird **per Service** optional). Tests: Catalogue-Pin (`TestSupportedServices`), Template-Existenz-Pin (Volume-Template für Keycloak NICHT pflicht), Render-Smoke. |
| T2 | **Per-Service-Probe-Mechanismus.** Heute ist der Detect-Pfad an drei Stellen Postgres-hardcoded — alle drei müssen pro Service konfigurierbar werden: (a) `hasRequiredEnvKeys` (`addservice_detect.go:182`) prüft literal `POSTGRES_USER`/`POSTGRES_PASSWORD`/`POSTGRES_DB` — wird zu `hasRequiredEnvKeysFor(svc, blockBody)` mit Service-Catalogue-Lookup (`postgres → [POSTGRES_USER, POSTGRES_PASSWORD, POSTGRES_DB]`, `keycloak → [KEYCLOAK_ADMIN, KEYCLOAK_ADMIN_PASSWORD]`); (b) `contentScanState` / `feedSubBlockEntry` matchen `postgres-data` als Volume-Ref-Literal und `POSTGRES_*` als Environment-Keys — der Scan-State wird per Service parametrisiert (`requiredEnvKeys`, `volumeRefLiteral`, `skipVolume`-Flag); (c) `inspectVolumeArtefact` (`addservice_detect.go:98`) liefert heute für jeden Service ohne Volume `needsRepair=true` → für Keycloak Endlos-Repair-Schleife; Fix: Service-Catalogue-Eintrag `volumeOptional: true` → `inspectVolumeArtefact` skippt komplett, gibt `false, nil` (no-repair) zurück. **Designentscheidung T2:** Service-Catalogue als kleines `serviceProbe`-Struct in `internal/hexagon/application/servicecatalogue.go` (oder als Tabelle in `addservice_detect.go`), beheimatet `requiredEnvKeys []string`, `volumeRefLiteral string`, `volumeOptional bool`, `composeTemplate / envTemplate / volumeTemplate` Pfade. `renderPostgresTemplates` zu `renderServiceTemplates(svc)` darüber. `executeRemove` (RemoveService) muss ebenfalls die Volume-Skip-Logik respektieren — heute liest es blind den Volume-Block; T2-Carveout: Volume-Removal bei `volumeOptional` skippen statt ErrServiceInconsistent. Tests: 100% Coverage auf den neuen Catalogue-Lookup, **Postgres-Snapshot-Pin (Byte-Identity der Postgres-Render-Outputs unverändert)**, Keycloak-Probe-Pin (Active-Detection für Keycloak ohne Volume → kein `needsRepair`). |
...
| T4 | **Closure.** READMEs (`add <service>`-Subcommand-Reference erwähnt jetzt `postgres \| keycloak`), CHANGELOG `## [Unreleased]` Added-Eintrag mit LH-FA-ADD-003 + LH-AK-003-Bezug + Hinweis auf flüchtige Default-Persistenz + Folge-Slice-Verweis, roadmap.md §v0.3.0-Tabelle markiert `slice-v1-keycloak` ✅ mit T1..T3-Hashes + Stand-Bump 4/5, Slice-Plan `open/` → `done/` mit Tranchen+Commit-Tabelle. `make docs-check` grün. |
