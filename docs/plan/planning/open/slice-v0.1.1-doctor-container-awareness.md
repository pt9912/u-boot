...
| T | Inhalt |
| - | ------ |
| T1 | Container-Detection-Helper in `internal/hexagon/domain/` (z. B. `domain.RuntimeEnvironment` mit `InContainer() bool` aus `/.dockerenv` / `/run/.containerenv`). Tests gegen tabellengetriebene Fixtures. |
| T2 | `internal/hexagon/application/doctor`: Host-Voraussetzungs-Checks im Container-Modus auf `Severity: Skipped` / `Status: info` umstellen mit klarer Hint-Zeile. `--strict` bleibt strikt, aber Container-skipped-Checks gelten nicht als Severity-Eskalation. |
| T3 | README.{md,de.md} Quickstart-Sektion + `doctor --help` um Container-vs-Host-Verhalten erweitern. CHANGELOG `## [0.1.1]`-Sektion anlegen. |
| T4 | Slice-Plan-Closure + carveouts/roadmap-Sync; Release-Tag `v0.1.1` durch Nutzer. |
