...
| --- | --- | --- |
| `driven.TemplateFiles.Open(ctx, name)` | Port existiert, eine Impl (`externaltemplates.Catalog`, `embed.FS`) | **zweite Impl** (FS-Resolver) + **Composite**, der via `domain`-Klassifikation an Katalog oder FS delegiert |
...
| `readTemplate` (apiVersion-Gate, `KnownFields`, `Validate`) | privat in `externaltemplates/catalog.go:194` | **teilen/extrahieren** für die FS-Metadaten-Validierung |
...
| # | Frage | Empfehlung |
| - | ----- | ---------- |
| (a) | **Name-vs-Pfad-Disambiguierung — Regel:** wann ist `--template X` ein Katalog-Name, wann ein lokaler Pfad? | Pfad, wenn `X` mit `./`, `../`, `/` oder `~` beginnt **oder** einen Pfad-Separator enthält; sonst Katalog-Name. Deterministisch, kein FS-Stat zur Klassifikation (vermeidet TOCTOU + überraschende Reklassifikation bei gleichnamigem Verzeichnis). Die Regel selbst ist als **reine `domain`-Funktion** (z. B. `domain.ClassifyTemplateRef(s)`) zu implementieren — pure, FS-frei, depguard-konform (domain überall importierbar), gezielt unit-testbar. |
...
| (c) | **Metadaten-Validierung lokaler Templates:** wie? | `readTemplate`-Logik (apiVersion-Gate + `KnownFields` + `Validate()`) in einen teilbaren Helfer extrahieren (T1) und im FS-Resolver wiederverwenden — ein fehlendes/ungültiges/unsupported `template.yaml` muss am Root genauso hart abweisen wie im Katalog (fail-fast). Fehlerklasse siehe (d). |
...
| T1 | **Fundament (pure/infra + Service):** (1) `readTemplate`/apiVersion-Gate/`KnownFields`/`Validate` aus `externaltemplates/catalog.go` in wiederverwendbaren Helfer ziehen, ohne den Katalog-Pfad zu brechen; (2) `domain`-Klassifikator (a) als reine Funktion + Unit-Tests; (3) neue Sentinels `driven.ErrTemplateInvalid` + `driving.ErrTemplateInvalid`; (4) `TemplateInitService` zwei umrissene Änderungen: `Open`-Mapping-Branch `ErrTemplateInvalid`→Exit 10 (d) **und** Symlink-Guard im `planRender`-Walk-Callback → `driving.ErrInvalidTemplatePath` Exit 10 (e). Symlink-Table-Test (Walk-Fixture mit Symlink → Exit 10, kein Write). `make gates` grün. |
| T2 | **FS-Resolver + Composite (Driven-Adapter):** neue Driven-Impl (z. B. `internal/adapter/driven/localtemplates/`), die `Open(ctx, path)` gegen `os.DirFS`/den `fs`-Adapter rooten, Root-Existenz/Verzeichnis-Check + `template.yaml`-Gate (→ `ErrTemplateNotFound` bzw. `ErrTemplateInvalid` aus T1) prüft. **Symlink-Policy liegt nicht hier**, sondern im Application-Walk-Loop (T1/(e)) — der Resolver liefert nur den gerooteten `iofs.FS`. Composite-`TemplateFiles` (a2), der via `domain`-Klassifikator an Katalog oder FS-Resolver delegiert. Table-driven Tests mit `fstest.MapFS` (inkl. malformed-metadata- und missing-`template.yaml`-Fälle). |
| T3 | **Wiring + End-to-End:** `main.go` verdrahtet **einen** Composite-Resolver (Katalog + FS); CLI `--template` bleibt unverändert (kein Business-Dispatch im CLI). Exit-Code-Mapping end-to-end gepinnt (not-found 10 / invalid 10 / render-IO 14). Mutex-Regeln (`--template` + `--dry-run`/`--diff`/`--devcontainer`/`--force`/`--backup`) aus `slice-v1-cli-json-dry-run-init` gelten unverändert. |
