...
Code-Review (xhigh-effort, 5 Angles + Sweep, gezielter Scope auf
`domain.TemplatePath` Edges / Render-Loop / `initFromTemplate`-
Order) hat fünf Findings ergeben. Im Followup-Commit gefixt
(gleicher Branch, kein eigener Slice):

| F | Datei | Befund | Fix |
| - | ----- | ------ | --- |
| F1 | `application/template_init.go` | Mid-Walk-Render-Failure ließ teilrenderierte Files auf Disk zurück (z. B. File 1 schreibt OK, File 2 fail render → File 1 bleibt liegen, Init kehrt mit Fehler). | Two-Phase-Render: Phase 1 läuft die Walk und rendert in-memory zu einer `[]renderedFile`-Liste; Phase 2 schreibt erst, wenn ALLE Renders erfolgreich. Renderfehler short-circuiten jetzt vor jedem Disk-Side-Effect. IO-Fehler in Phase 2 (Disk-Full o. ä.) können noch Partial-State erzeugen, aber das ist die seltene Klasse. Neuer Test `TestTemplateInitService_RenderFailureMidWalkNoPartialWrite` pinnt `len(fs.writes) == 0` bei 2-File-Fail. |
| F2 | `domain/template_path.go` | Backslash-Pfade umgingen den `..`-Detektor — `docker\..\..\etc\passwd` wurde durch `strings.Split(raw, "/")` als Single-Segment behandelt; auf Windows hätte `filepath.Join` die Backslashes als Separatoren interpretiert und das Project-Tree verlassen. | `strings.ContainsRune(raw, '\\')` rejected jetzt jeden Backslash up-front (POSIX akzeptiert ihn als Filename-Zeichen, aber unsere Domain ist forward-slash-only). Neuer Test-Case + ein bestehender Test (`D:\bar`) auf die neue Fragment-Erwartung "backslash" angepasst (Order: backslash check fires before drive-letter check). |
| F3 | `application/initproject.go` | `checkSoftExisting` feuerte auch im `initFromTemplate`-Pfad — Nutzer mit Leftover `docker/`/`scripts/`/`docs/` aus einem vorherigen Versuch wurden beim Wiederholen mit `--template basic` durch den Soft-Existing-Prompt blockiert, obwohl `--template` explizit Fresh-Init-Intention signalisiert. | `checkSoftExisting`-Aufruf aus `initFromTemplate` entfernt; Doc-Comment erklärt warum (Soft-Detection ist für die Default-Pfad-Ambiguität "ist das ein bestehendes Projekt?"; `--template` löst diese Ambiguität explizit). Die Hard-Existing-Check via `u-boot.yaml`-Existenz → `ErrProjectExists` bleibt der einzige Safety-Net. Neuer Test `LeftoverStructureDirsDoNotBlock`. |
| F4 | `domain/template_path.go` | NUL-Bytes wurden in den Pfad durchgereicht; das Schreiben failt deterministisch auf OS-Ebene (Linux: EINVAL), aber die Domain-Boundary war unklar. | `strings.ContainsRune(raw, 0)` rejected jeden NUL-Byte; Test-Case pinnt das Verhalten. |
| F5 | `application/template_init.go` | `iofs.WalkDir`-Layer-Errors (`walkErr`-Parameter) propagierten unklassifiziert; ein zukünftiger Filesystem-Adapter mit Race / Permission-Fehler hätte Exit-Code 1 statt 14 gemeldet. | `if walkErr != nil` wrapt jetzt mit `driving.ErrTemplateRender` über Multi-%w; CLI-Klassifikation greift jetzt auch für die Walk-Layer-IO-Pfade. |

Gates nach Followup grün: lint + test + coverage-gate 90.10% +
docs-check. E2E-Smoketest `diff -r` zwischen Default-Pfad und
`--template basic`-Pfad bleibt leer (keine Wire-Shape-Regression).
...
- **`--template ./pfad`-Filesystem-Auflösung**: gehört zum
  Later-Slice [`slice-later-local-templates`](slice-later-local-templates.md) ([ADR-0009](../../adr/0009-template-format-yaml-files.md) §Folgepunkte 3).
...
- ADR: [ADR-0009](../../adr/0009-template-format-yaml-files.md)
  ...
  `embed.FS`-Layout) + §Folgepunkte ([`slice-v1-template-init`](slice-v1-template-init.md) mit
  T5 ✅).
...
- Phase: V1 (nach [`slice-v1-template-list`](slice-v1-template-list.md)).
