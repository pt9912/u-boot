...
| `u-boot template list --json` (`internal/adapter/driving/cli/template.go`, slice-v1-cli-json-dry-run-doctor T3+T4): das lokale `--json`-Flag ist durch das Root-PersistentFlag ersetzt, das Output-Format bleibt aber die existierende `templateJSON`-Array-Struktur — **nicht** Minimalkontrakt-konform (LH-NFA-USE-004 §1841 fordert `status`/`command`/`subcommand`/`diagnostics`/`exitCode`-Envelope). | temporär | Folge-Slice `slice-v1-cli-json-dry-run-template` (Cluster-Platz 9 in [`slice-v1-cli-json-dry-run.md`](slice-v1-cli-json-dry-run.md) §Per-Command-Folge-Slices) migriert den Output auf den `cliJSONEnvelope` mit `command: "template"` + `subcommand: "list"` + `data: []templateJSON`. Plan-Anker in [`slice-v1-cli-json-dry-run-doctor`](../done/slice-v1-cli-json-dry-run-doctor.md) §Akzeptanzkriterien §`template list --json`-Schnitt (bewusst nicht in diesem Slice geschlossen, weil Envelope-Migration semantisch zum Platz-9-Slice gehört). |
...
| CLI-JSON-Envelope-Pattern-Drift in `add`/`init`/`generate` (R15-Cross-Slice-1 aus slice-v1-cli-json-dry-run-remove): `cli/add.go:79`, `cli/generate.go:78` nutzen rohes `cobra.ExactArgs(1)` ohne JSON-Envelope-Hook (Spec §1841-Verletzung im NoPositionalArg- und TooManyArgs-Pfad analog R13-MED-1 vor T7). Init/add/generate-Mapper haben keinen BaseDir-Sanitizer für `diagnostic.message` (Path-Leak-Risiko analog R14-MED-1 vor T7). | temporär | Folge-Slice [`slice-v1-cli-json-envelope-consolidation`](../open/slice-v1-cli-json-envelope-consolidation.md) (`open/`, Status `on hold pending trigger`) konsolidiert die zwei Patterns Cluster-weit: (1) Custom-`Args`-Validator-Pattern analog `validateRemoveArgs` für add/generate, (2) `baseDirSanitizedError`-Wrapper-Pattern bzw. Helper-Extraktion in shared `cli/`-Sub-Package. Sub-Decision 2 (Helper-Heim) ist nach up-down T5 festgelegt: `cli/sanitize.go` File-Heim. Trigger: Folge-Slice-Sequenz erreicht 7/9 oder 8/9 (Konsolidierung lohnt sich erst wenn alle modifying-Subcommands JSON-Envelopes tragen), ODER Real-World-Beschwerde über fehlende Symmetrie. |
