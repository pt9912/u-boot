...
| Build-Tag-Pfad `//go:build docker` für Adapter-Integrationstests (`spec/architecture.md` §5): Sub-T1..Sub-T4 sind implementiert (6 Verhaltens-Pins über docker-adapter / application / e2e plus `make test-docker` + GitHub-Actions-`integration-docker`-Job mit `continue-on-error: true`). Carveout-Aufhebung pending: 3× grün auf `main` + erster Lauf ohne `continue-on-error` (Audit-Block siehe Slice) | temporär | [`in-progress/slice-m6-docker-integrationstests.md`](../in-progress/slice-m6-docker-integrationstests.md) |
