...
| M8 `u-boot config` | In progress | `config get`/`set`/Anzeigen (`LH-FA-CONF-001..005`), Schema-Validierung. Letzter MVP-blockierender Slice. T1 ✅ (`f531e7e`): `domain.ConfigPath` mit 3-Pfad-Whitelist + `WriteAllowed`-Flag. T2 ✅ (`d3fa294`): `ConfigUseCase`-Port (3 Methoden), `ConfigService`-Skeleton mit Stub-Pin, 4 neue Sentinels, shared `checkProjectInitialized`-Gate. | [`slice-m8-config`](slice-m8-config.md) |
