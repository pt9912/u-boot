...
| M6 `u-boot up` / `down` | Done | Compose-Wrapper (`LH-FA-UP-001..004`), Healthcheck-Polling, `--timeout`, `--volumes`, CLI-Subcommands + Status-Tabelle. Alle 7 Tranchen in `done/`. Reale Docker-Engine-Verhalten via Carveout-Slice [`slice-m6-docker-integrationstests`](../open/slice-m6-docker-integrationstests.md). | [`slice-m6-up-down`](slice-m6-up-down.md) |
...
1. **M6 up/down**: **In progress** (siehe [`slice-m6-up-down.md`](slice-m6-up-down.md)). Slice-Plan in `in-progress/`, review-gehärtet (vier Review-Runden: Sentinel-Schichtung mit `errors.Is`-Durchleitung, `containerState`-Enum mit Dead-Allowlist + soft-unknown, `--timeout=0` fire-and-forget mit Triple-Pin, Restart-Loop-Counter, robustes Compose-Port-Parsing, §235-vs-§254-CLI-Pfade getrennt). 1. **M6 up/down**: **Done**. Alle 7 Tranchen abgeschlossen: T1 ✅ (`9f8badd`, Domain + Driving-Ports + Sentinels), T2 ✅ (`84a676c`, DockerEngine driven-port + Adapter mit preflight-Klassifikation), T3 ✅ (`1e5ef18`, NetProbe driven-port + Adapter + application-no-net depguard, alle 9 Regeln verifiziert), T4 ✅ (`1351cfb` + foundation `9101bdc`, UpService mit Polling-Loop, 21 Pin-Tests + 3 Review-Fixes via `d1deee5`), T5 ✅ (`a46bec3`, DownService mit §T5 truth-table + ConfirmRemoveVolumes-Port-Erweiterung), T6 ✅ (`4a7e60d`, CLI Output-Layer mit tabwriter-Status-Tabelle + Diagnostics + Down-Success), T7 ✅ (`6d9aa88`, up/down Cobra-Subcommands + cli.New 5-Use-Cases + main.go-Wiring + 8 Pin-Tests + ExitCode-Mapping für codes 11/12). Coverage 91.20%. Reale Docker-Engine-Verhalten via Carveout-Slice [`slice-m6-docker-integrationstests`](../open/slice-m6-docker-integrationstests.md). Aktive Tranche: **M7** (`u-boot generate`).
