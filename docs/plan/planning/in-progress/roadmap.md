...
| M5 `u-boot add postgres` | In progress | PostgreSQL-Add-on (`LH-FA-ADD-001..002`, `LH-FA-ADD-005`), services-Schema in u-boot.yaml, Compose-Block, `.env.example`-Block, Healthcheck, State-Machine für Re-Aktivierung + Inkonsistenz-Erkennung. Detail: [`slice-m5-add-postgres.md`](slice-m5-add-postgres.md). **Stand:** T1 ✅ `995726a` (services-Schema + ServiceName/ServiceState-Domain); T2 ✅ `f054986` (AddServiceUseCase driving port + 3 sentinels); T3 ✅ `4091ac9` (AddServiceService skeleton + detectServiceState + PatchScalar-Port-Erweiterung; executeAdd-Stub für T4); T4a ✅ `6052b71` (Compose split-block scaffold + renderManagedBlockOnly + Ensure-Scaffold-Pass); T4b ✅ `cca1254` (PatchMappingEntryYAML + LocateMarkedEntry Port-Erweiterungen + yaml-Adapter mit Hash-Marker-Scanner); T4c ✅ `529be1c` (postgres-Templates + executeAdd-Plan-and-Execute + Active-Repair-Pfad); T5 ✅ `23209cd` (LH-FA-ADD-005-State-Machine-End-to-end-Tests + Round-Trip-Szenarien); T6 ✅ `7e7bbcd` (CLI-Subcommand `u-boot add <service>` mit Cobra-Wiring + Exit-Code-Mapping); T7 offen | [`slice-m5-add-postgres`](slice-m5-add-postgres.md) |
...
1. **M5 add-postgres**: 7 Tranchen geplant (siehe [`slice-m5-add-postgres.md`](slice-m5-add-postgres.md)). T1 (Schema + Domain-Types), T2 (AddServiceUseCase + Sentinels), T3 (AddServiceService-Skeleton + State-Detection + PatchScalar-Port-Erweiterung; executeAdd-Stub) und T4a (Compose split-block scaffold + renderManagedBlockOnly + Ensure-Scaffold) und T4b (PatchMappingEntryYAML + LocateMarkedEntry Port-Erweiterungen mit yaml-Adapter) und T4c (postgres-Templates + executeAdd-Plan-and-Execute + Active-Repair) und T5 (LH-FA-ADD-005-State-Machine-End-to-end-Tests + Round-Trip-Szenarien) und T6 (CLI-Subcommand `u-boot add <service>` mit Cobra-Wiring + Exit-Code-Mapping) abgeschlossen. T4 vollständig done. Aktive Tranche: T7 (Closure: doctor-Integration für services.<name>.enabled-Check + Slice-Move nach done/).
