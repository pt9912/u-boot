...
| [`slice-later-local-templates`](../open/slice-later-local-templates.md) | `open/`, **in progress — T1 (`66c347d`), T2 (`87a8704`), T3 (`5031b5f`) done** (Later) | `u-boot init --template ./pfad` **funktional verdrahtet** (Composite in `main.go`, End-to-End-Exit-Code-Matrix gepinnt). Verbleibend T4 (README/CHANGELOG/ADR-Doku + Acceptance), T5 (Closure). `LH-FA-TPL-003`. |
