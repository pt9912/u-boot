...
| M4 `u-boot doctor` | In progress | Lokale Voraussetzungen prüfen (`LH-FA-DIAG-001..004`), Severity-Klassifikation, Repair-Hints. Vorbereitende Slices erledigt: `slice-m4-soft-existing-detection`, `slice-m4-logging-port`. Detail: [`slice-m4-doctor.md`](slice-m4-doctor.md). **Stand:** T1 ✅ `bcf684f` (domain + driving port); T2 ✅ `19423d9` (service skeleton + write-perms); T3 ✅ `a447d76` (Git/Docker/Compose-Probes + semver); T4 ✅ `41de6b4` (u-boot.yaml validation); T5 ✅ `41eccb5` (compose.yaml validation); T6 ✅ `9182aba` (devcontainer.json/Dockerfile validation + JSONC parser); T7 offen | [`slice-m4-doctor`](slice-m4-doctor.md) |
...
1. **M4 doctor**: 7 Tranchen geplant (siehe [`slice-m4-doctor.md`](slice-m4-doctor.md)). Aktive Tranche: T1 (Domain + Driving-Port).
