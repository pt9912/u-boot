...
| M5 `u-boot add postgres` | In progress | PostgreSQL-Add-on (`LH-FA-ADD-001..002`, `LH-FA-ADD-005`), services-Schema in u-boot.yaml, Compose-Block, `.env.example`-Block, Healthcheck, State-Machine für Re-Aktivierung + Inkonsistenz-Erkennung. Detail: [`slice-m5-add-postgres.md`](slice-m5-add-postgres.md). **Stand:** T1..T7 offen | [`slice-m5-add-postgres`](slice-m5-add-postgres.md) |
