...
| [`slice-later-local-templates`](../open/slice-later-local-templates.md) | `open/`, **in progress — T1 (`66c347d`), T2 (`87a8704`) done** (Later) | Fundament + FS-Resolver/Composite gebaut (`templateyaml`, `domain.ClassifyTemplateRef`, `ErrTemplateInvalid`, Symlink-Guard, `localtemplates`). Noch nicht in `main.go` gewired. Verbleibend T3–T5 (Wiring, E2E/Docs, Closure). `LH-FA-TPL-003`. |
