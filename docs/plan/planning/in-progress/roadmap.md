...
| M8 `u-boot config` | In progress | `config get`/`set`/Anzeigen (`LH-FA-CONF-001..005`), Schema-Validierung. Letzter MVP-blockierender Slice. T1 ✅ (`f531e7e`): `domain.ConfigPath`-Whitelist. T2 ✅ (`d3fa294`): `ConfigUseCase`-Port + Skeleton, 4 Sentinels. T3 ✅ (`23952b2`): Get + Show Handler + `ErrConfigValueNotSet`-Sentinel; Stub-Pin reduziert auf Set. | [`slice-m8-config`](slice-m8-config.md) |
