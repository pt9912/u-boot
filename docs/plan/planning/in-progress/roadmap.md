...
| M2c CI | Done | GitHub-Actions-CI (`LH-QA-003` auf konkret gehoben), `.github/workflows/ci.yml` mit Jobs `gates` + `security-gates` (beide PR-blockierend), SHA-pinned Actions, Docker-only, ADR-0004. Drittes Image-Scan-Job (`image-scan (trivy HIGH+CRITICAL)`) später mit [`slice-v1-release-pipeline`](../done/slice-v1-release-pipeline.md) T3 (`8212889`) ergänzt; LH-QA-003-Pflichtmenge ist heute drei Jobs. | [`slice-m2c-ci-pipeline`](../done/slice-m2c-ci-pipeline.md) |
