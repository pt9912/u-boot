...
1. **M5 add-postgres**: 7 Tranchen geplant (siehe [`slice-m5-add-postgres.md`](slice-m5-add-postgres.md)). T1 (Schema + Domain-Types), T2 (AddServiceUseCase + Sentinels) und T3 (AddServiceService-Skeleton + State-Detection + PatchScalar-Port-Erweiterung; executeAdd-Stub) abgeschlossen. Aktive Tranche: T4 (PostgreSQL-Templates + Write-Pfad inkl. Compose-Scaffold-Restrukturierung und PatchMappingEntryYAML-Port-Erweiterung).
