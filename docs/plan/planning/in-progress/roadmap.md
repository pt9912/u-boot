...
1. **M5 `u-boot add postgres`**: PostgreSQL-Add-on (`LH-FA-ADD-001..005`), Compose-Block, `.env.example`-Block, Healthcheck. Erweitert `u-boot.yaml` um das fehlende `services:`-Schema (entsperrt die in M4 deferred forwardPorts-Konsistenz und die `devcontainer.enabled`-abhängige Severity-Eskalation).
