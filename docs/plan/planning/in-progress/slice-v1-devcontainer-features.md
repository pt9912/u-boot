...
**Go-LOC-Summe T1-T6 ≈ 1070** (geschätzt T1 150 + T2 100 + T3 200 + T4 250 + T5 270 + T6 100). **Real-LOC bisher:** T1 ≈ 230 (+53 %, depguard-`application-no-net`-Constraint zwang manuellen URL-Parser), T2 ≈ 84 (−16 %, unter Budget), T3 ≈ 115 (−43 %, deutlich unter Budget — Render-Pipeline reused M7-Managed-Block + bestehende Test-Infrastruktur). **T1+T2+T3-Summe ≈ 429 LOC < 600-Re-Check-Schwelle** — keine T4-Rekalibrierung nötig. **T4-Plan bei 250 LOC bleibt:** Projektion T1-T4 = 429 + 250 = **679 LOC < 800-Schwelle** → Doctor-Teil-B bleibt komfortabel in T5; kein Folge-Slice-Carveout. Vergleichswerte aus
[`slice-m5-add-postgres`](../done/slice-m5-add-postgres.md) +
