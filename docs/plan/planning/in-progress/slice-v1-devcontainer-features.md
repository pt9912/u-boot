...
**Go-LOC-Summe T1-T6 ≈ 1070** (geschätzt T1 150 + T2 100 + T3 200 + T4 250 + T5 270 + T6 100). **T1-Real-LOC ≈ 230** (53 % über Schätzung — primär getrieben durch `application-no-net`-Depguard-Constraint, der einen manuellen URL-Parser erzwungen hat). **Aktualisierte T1-T4-Projektion bei gleichem Skalierungs-Faktor: 230 + 153 + 306 + 383 ≈ 1070 LOC** — würde die 800-Schwelle reißen. **Re-Check-Auslösebedingung verschärft:** wenn nach T2-Abschluss (`T1+T2` real > 380 LOC) und nach T3-Abschluss (`T1+T2+T3` real > 600 LOC) die T4-Schätzung neu kalibrieren; bei projizierter T1-T4 > 800 LOC Folge-Slice `slice-followup-devcontainer-features-drift-doctor` in `open/` anlegen *vor* T5-Start. Vergleichswerte aus
[`slice-m5-add-postgres`](../done/slice-m5-add-postgres.md) +
