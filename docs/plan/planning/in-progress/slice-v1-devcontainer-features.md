...
- 🟡 **Doctor-Integration Teil B (über Spec hinaus,
  ...
  nach T4:** T1-T4-Real-LOC ≈ 939 > 800-Schwelle (siehe LOC-
  Bilanz unter den Tranchen). Vor T5-Start wird der
  Folge-Slice `slice-followup-devcontainer-features-drift-doctor`
  in `open/` angelegt; Teil B wandert dorthin, T5 in diesem
...
**Go-LOC-Summe T1-T6 ≈ 1070** (geschätzt T1 150 + T2 100 + T3 200 + T4 250 + T5 270 + T6 100). **Real-LOC nach T4:** T1 ≈ 230 (+53 %), T2 ≈ 84 (−16 %), T3 ≈ 115 (−43 %), T4 ≈ 510 (+104 %, vier neue Config-Pfade × 5 Pipeline-Stages plus List-Pfad-Marshal-Rewrite-Sonderfall plus Spec-§714-Constraint-Validierung auf 3 Commands plus Komplexitäts-Refactoring). **T1-T4-Summe ≈ 939 LOC > 800-Schwelle** — **Carveout-Trigger gefeuert.** Vor T5-Start ist gemäß Plan-Vertrag der Folge-Slice `slice-followup-devcontainer-features-drift-doctor` in `open/` anzulegen und Doctor-Teil-B (`devcontainer.features.drift`) dorthin auszulagern. T5 in diesem Slice schrumpft damit auf Teil A (`devcontainer.features.allowlist`, ~120 LOC). Vergleichswerte aus
[`slice-m5-add-postgres`](../done/slice-m5-add-postgres.md) +
