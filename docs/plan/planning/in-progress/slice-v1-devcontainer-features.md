...
**Go-LOC-Summe T1-T6 ≈ 1070** (geschätzt T1 150 + T2 100 + T3 200 + T4 250 + T5 270 + T6 100). **Real-LOC nach T4 + Review-Followup:** T1 ≈ 230 (+53 %), T2 ≈ 84 (−16 %), T3 ≈ 115 (−43 %), T4 ≈ 510 (+104 %), T4-Review-Followup R1..R6 ≈ 70 (Härtung Spec §720/§1353, atomare Reihenfolge, UX-Hinweise). **T1-T4-Summe inkl. Followup ≈ 1009 LOC > 800-Schwelle** — **Carveout-Trigger gefeuert.** Vor T5-Start ist gemäß Plan-Vertrag der Folge-Slice `slice-followup-devcontainer-features-drift-doctor` in `open/` anzulegen und Doctor-Teil-B (`devcontainer.features.drift`) dorthin auszulagern. T5 in diesem Slice schrumpft damit auf Teil A (`devcontainer.features.allowlist`, ~120 LOC). Vergleichswerte aus
[`slice-m5-add-postgres`](../done/slice-m5-add-postgres.md) +
