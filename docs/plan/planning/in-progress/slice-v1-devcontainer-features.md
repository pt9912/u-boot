...
**Go-LOC-Summe T1-T6 ≈ 1070** (geschätzt T1 150 + T2 100 + T3 200 + T4 250 + T5 270 + T6 100). **Real-LOC bisher:** T1 ≈ 230 (+53 %, depguard-`application-no-net`-Constraint zwang manuellen URL-Parser), T2 ≈ 84 (−16 %, unter Budget). **T1+T2-Summe ≈ 314 LOC < 380-Re-Check-Schwelle** — keine T4-Rekalibrierung nötig nach T2. **Aktualisierte T1-T4-Projektion:** wenn T3/T4 auf Plan bleiben → 314 + 200 + 250 = 764 LOC; wenn T3/T4 mit T1-Skalierungs-Faktor +53 % skalieren → 314 + 306 + 383 = 1003 LOC. Re-Check vor T4-Start bleibt verbindlich: wenn T1+T2+T3 real > 600 LOC, dann T4-Schätzung neu kalibrieren; bei projizierter T1-T4 > 800 LOC Folge-Slice `slice-followup-devcontainer-features-drift-doctor` in `open/` anlegen *vor* T5-Start. Vergleichswerte aus
[`slice-m5-add-postgres`](../done/slice-m5-add-postgres.md) +
