...
| T5  | **Doctor-Checks (nur Teil A; Teil B ausgelagert).** Carveout-Trigger gefeuert nach T4 (T1-T4 ≈ 1009 LOC > 800). Check-ID `devcontainer.features.allowlist` (Spec-mandatiert): `devcontainer.features.<name>.source` referenziert nur Quellen aus `featureSources.allow` *oder* `features.<name>` ist Katalog-Eintrag (kein `source:`-Override). Severity-Klassifikation analog M5. Teil B (`devcontainer.features.drift`) wandert in [`slice-followup-devcontainer-features-drift-doctor`](../open/slice-followup-devcontainer-features-drift-doctor.md). | ~120 (nur Teil A) |
...
**Go-LOC-Summe T1-T6 ≈ 920** (geschätzt nach Carveout: T1 150 + T2 100 + T3 200 + T4 250 + T5 120 + T6 100; Teil-B-Schätzung des Folge-Slice ≈ 200 LOC nach Plan-Followup-S1/S2, siehe dort). **Real-LOC nach T4 + Review-Followup:** T1 ≈ 230 (+53 %), T2 ≈ 84 (−16 %), T3 ≈ 115 (−43 %), T4 ≈ 510 (+104 %), T4-Review-Followup R1..R6 ≈ 70 (Härtung Spec §720/§1353, atomare Reihenfolge, UX-Hinweise). **T1-T4-Summe inkl. Followup ≈ 1009 LOC > 800-Schwelle** — **Carveout-Trigger gefeuert.** Folge-Slice [`slice-followup-devcontainer-features-drift-doctor`](../open/slice-followup-devcontainer-features-drift-doctor.md) (~200 LOC nach Plan-Followup-Schärfung) in `open/` angelegt; Doctor-Teil-B wandert dorthin. T5 in diesem Slice realisiert nur noch Teil A (`devcontainer.features.allowlist`, ~120 LOC). Vergleichswerte aus
[`slice-m5-add-postgres`](../done/slice-m5-add-postgres.md) +
