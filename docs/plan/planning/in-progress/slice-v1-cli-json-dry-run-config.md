...
> **Status:** `in-progress/` — **T3 done (2026-06-08, Application-
> Layer)**, `make gates` grün (Coverage 91.20 %). T2+T3 geliefert:
> Port-Felder `ConfigSetRequest.PreviewMode`/`SilenceLogger` +
...
> `configSetFlags.JSON`/`Quiet` read-through (T2); Multi-`%w`-FS-
> Wraps + Sentinel-Split-Wiring + SilenceLogger-Branch + Orphan-
> WARN→`Warnings` (Dual-Emission) in `application/config.go` (T3).
> **Verschoben nach T4**: `PreviewMode`-Handling (`fsFactory` +
> `selectFS` + Write-Routing) zusammen mit `WithFactory`-Konstruktor
> + Composition-Root (sonst selectFS-Branch uncovered). **Verschoben
> nach T5** (Lint-/Behavior-Grund): `configGetFlags`/`configShowFlags`
> + `DryRun`/`Diff`-Felder + `--dry-run`/`--diff`-Flag-Registrierung.
> Nächster Schritt: **T4** (PreviewMode-Cluster + Composition-Root).
> R1+R2+R3-Adressierung gefahren: R1=4+6+4, R2=3+6+4, R3=0+4+4 —
> Asymptote erreicht; T0-Sub-Decisions (a)-(p) komplett mit
> R3-festgezurrten Beschlüssen für (a)/(b)/(g)/(o)/(p); vier
...
| T4 | **PreviewMode-Cluster (aus T3 verschoben)** + **Composition-Root-Erweiterung** (R2-HIGH-2-Adressierung — ursprüngliche "entfällt"-Aussage war falsch): `ConfigService.fsFactory`-Feld + nil-safe `selectFS(mode)`-Methode (analog `AddServiceService`); `Set` + `setFeatureSourcesAllow` routen ihren `WriteFile` über `fs, _ := s.selectFS(req.PreviewMode)` (Reads bleiben auf Production-`s.fs`). `NewConfigServiceWithFactory(fsFactory, yaml, logger)`-Konstruktor neu (analog `NewAddServiceServiceWithFactory` etc.) + `configFSFactory := newPreviewFSFactory(fsAdapter)`-Zeile in `cmd/uboot/main.go:121-138`-Block + Konstruktor-Wechsel `NewConfigService` → `NewConfigServiceWithFactory`. Heute ist `ConfigService` der **einzige Modifying-Service ohne `WithFactory`-Variante**. Dry-Run-Acceptance-Tests (selectFS-Nicht-nil-Branch + kein WriteFile bei PreviewDryRun) landen hier, damit der Branch covered ist. | ~60 (≈40 Composition-Root + ≈20 PreviewMode-aus-T3) | T3 |
